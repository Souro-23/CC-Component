...
export class CourseContent extends Component {
  ...
  handleSubtopicMove = (topicId, subtopicArray) => {
    var newTopics = this.state.topics.map((topic, index) => {
      if (topic.topic.sno === topicId) {
        return {
          topic: {
            sno: topicId,
  ...
  handleMove = (id, direction) => {
    ...
    if (position < 0) {
      ...
      (direction === DOWN && position === topics.length - 1)
    ) {
    ...
    console.log(newtopics, "after");
    this.setState({ topics: newtopics });
  };

  deleteTopic(topic, key) {
    const deletedTopic = this.state.topics.filter(
      (topic, index) => index !== key
    );
    this.setState({
      topics: deletedTopic,
    });
        ...
        return (
          <div key={index} id={topic.topic.sno}>
            <div style={{ display: "flex", alignItems: "center" }}>
              <ion-icon
                onClick={() => this.handleMove(topic.topic.sno, UP)}
              ...
              <div className={classes.Topic}>
                <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  />
                  <h4 className={classes.topicTag}>{topic.topic.name}</h4>
                </div>
                <DeleteOutlined
                  style={{ color: "white" }}
                  onClick={() => this.showDeleteConfirm(topic.topic, index)}
                />
              </div>
              <ion-icon
                style={{ cursor: "pointer" }}
                onClick={() => this.handleMove(topic.topic.sno, DOWN)}
                size='large'
                name='chevron-down-outline'></ion-icon>
            </div>
            <SubTopicList
              topicId={topic.topic.sno}
              key={index}
              subtopics={topic.subtopics}
              handleSubtopicDrag={this.handleSubtopicMove}
            />
